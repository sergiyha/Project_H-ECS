...
чанк, що створює нове сміття(алокацій) навантаження за утилізацію якого потім падає на CLR. В Project_H ECS ви не побачите жодної алокації, але додавання і
видалення компонентів викунується на достатньо високій швидкості навіть на декількох тисячах ентіті, що є дуже важливим в ході розробки геймплею на ECS. Цей
...
- `Storage.MaxEntitiesCount = 100_000` - також можливе збільшення
